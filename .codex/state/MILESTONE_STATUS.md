...
- M1 status: `ACTIVE / plan_version 23 / next_batch_sequence 12`
...
- `M1-B011`: `IMPLEMENTING / LINEAGE_CLEAN_CONTENT_RECONSTRUCTION`, depends only on `M1-B001`
...
- B011 implementation authorization: `OPEN`
- B011 product reconstruction: `NOT_YET_COMMITTED`
- Active implementation WIP: `M1-B011`
...
- Next gate: `M1-B011-LINEAGE-CLEAN-RECONSTRUCTION-IMPLEMENT`
...
`M1-B002` 的 `BLOCKED` 状态是对 independently established historical evidence 的治理投影，保持 inactive / not resumed。`M1-B010` 是 historical semantic predecessor；其完成状态只投影已接受历史事实，旧 integration 已由 `M1-B011` clean reconstruction 取代。当前 tree 不声明或提供 B010 capability；本次 PLAN 只开放 B011 IMPLEMENT 窗口，产品重建尚未提交或验收。
