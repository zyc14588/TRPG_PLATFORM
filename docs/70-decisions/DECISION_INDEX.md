...
| 决策 ID | 状态 | 标题 | 范围 | 结论 |
|---|---|---|---|---|
| `R25-A01` | `ACTIVE` | Executor/profile 专属上下文策略 | `GOVERNANCE_CONTROL_PLANE` | Context budget 按 executor/model profile 决定；R24-A08 的全局统一阈值被部分覆盖，R23-A05 的 context-overflow 强制拆批只适用于 enforcement-enabled profile。 |
| `R25-A02` | `ACTIVE` | Codex 仅记录上下文遥测 | `GOVERNANCE_CONTROL_PLANE` | codex/codex-default 不启用项目级 context soft/hard limit；可测量 route bytes 和显式容量下的 ratio，但不因 55%/70% 或 ratio 强制拆批，Section ID、渐进披露、禁止批量读取、binding、stale route、schema 和 target 校验继续强制；不声明模型物理上下文无限。 |
| `R25-A03` | `ACTIVE` | OpenCode deepseek-v4-flash 强制预算 | `GOVERNANCE_CONTROL_PLANE` | opencode/opencode-deepseek-v4-flash 必须显式提供实际 capacity；ratio 小于等于 0.55 为 NORMAL，超过 0.55 且不超过 0.70 时缩减 on-demand Section、无法继续降低则进入 review state，超过 0.70 HARD FAIL 并要求缩小 route 或拆 batch；projectctl 不猜测容量。 |
| `R25-A04` | `ACTIVE` | 未知上下文 profile fail closed | `GOVERNANCE_CONTROL_PLANE` | 未知 executor/model profile 返回 PROFILE_REQUIRED，不得自动继承 Codex unrestricted policy。 |

## R26

| 决策 ID | 状态 | 标题 | 范围 | 结论 |
|---|---|---|---|---|
| `R26-A01` | `ACTIVE` | Governance maintenance 独立 target | `GOVERNANCE_CONTROL_PLANE` | Governance control-plane maintenance 不属于 milestone product batch，使用可验证的 GOV-[A-Z0-9-]+ 标识。 |
| `R26-A02` | `ACTIVE` | 四模式保持不变 | `GOVERNANCE_CONTROL_PLANE` | 不新增第五种 mode；PLAN、IMPLEMENT、ACCEPT、REPAIR 保持不变，ACCEPT/REPAIR 可选择 milestone batch 或 governance-maintenance target，IMPLEMENT 不可选择 governance maintenance。 |
| `R26-A03` | `ACTIVE` | Governance maintenance 机器契约 | `GOVERNANCE_CONTROL_PLANE` | 每个 governance maintenance 必须有 tracked CONTRACT.yaml，包含 maintenance_id、objective、reason、source blocker、allowed/forbidden scope、normative references、acceptance、tests、stop conditions 和 status；不得借 GOV task 实现产品功能。 |
| `R26-A04` | `ACTIVE` | 一次性 Maintenance Bootstrap | `GOVERNANCE_CONTROL_PLANE` | 只有控制面无法为自身生成合法 route 的 bootstrap deadlock 且有明确授权、固定 scope 和审计记录时，才允许一次性仓库外 Bootstrap；必须修复正式 route，并在正式 REPAIR route 通过检查后退休，后续同类任务只能使用正式 route。 |
