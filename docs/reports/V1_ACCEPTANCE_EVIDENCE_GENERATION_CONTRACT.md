...
V1 的 17 项定义以
`docs/acceptance/V1_ACCEPTANCE_EVIDENCE_MATRIX.md` 为唯一来源。候选矩阵不得在仓库中手工填入
...
- canonical 定义 hash 匹配，行号恰为 1–17，且没有重复或缺失；
...
- 行 command 精确等于 canonical 定义中的权威命令，evidence 必须实际执行该命令（复合
