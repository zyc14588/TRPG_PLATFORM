...
| AUD-036 | Fork 忽略关键字段，只写 parent snapshot 行，没有子 Campaign 实体；默认角色查询还可能混入 `keeper_only` 行 | `fork_canon_lineage.rs`；`preview_campaign_fork`、`record_campaign_fork`、materialization/replay；角色与当前角色卡双层 Visibility 过滤；P08 两个 migration | domain tests 拒绝 self-fork、坏/不匹配 hash 和未授权私密 scope；真实 DB 证明 `keeper_only` 角色/角色卡 sentinel 被排除，并从 ended source 创建 child scenario、character/sheet、session、scenes、manifest，删除后重放一致且 source 不变 | CLOSED_PASS |
