...
| 第十四轮，1 项 | `CombatantState` 唯一公开构造器只接收 `max_hp` 并强制满血/`ABLE`，导致上一遭遇留下的受伤、MajorWound、Dying/Dead 角色进入新战斗时被静默治愈 | 新增经不变量校验的 `CombatHealth(current_hp,max_hp,condition)` 值对象及 `CombatantState::health()` 传递接口；构造器必须显式接收持久化健康快照，规则与独立领域初始态校验允许合法伤势并拒绝 HP/condition 矛盾；跨遭遇 MajorWound/Dying 回归、完整 primary/Witness、workspace check/Clippy 与 34 目标 Semgrep | FIXED_LOCALLY_RERUN_PENDING |
