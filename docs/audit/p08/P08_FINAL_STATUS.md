...
| 验收项 | 代码与真实证据 | 状态 |
| --- | --- | --- |
| MajorWound 持续 | 聚合保存 prior condition；后续小伤或护甲全吸收不会清除；医疗必须由当前可行动治疗者以持久化 First Aid/Medicine 目标和服务端骰尝试，失败也留痕并消费回合/骰；成功 First Aid 可把 0 HP `Dying` 稳定为 1 HP `MajorWound`，不能用 Medicine 跳过急救，也不能把幸存者直接伪造成 `Able`；每次正式 Combat health/condition 变化都会创建确定性、锁定且保持私密 Visibility 的新 Character Sheet version 并推进 Character，删除投影后可从正史重放 | PASS |
...
非数据库套件 61 项、workspace check 与严格 Clippy 已通过。修复提交
`fc252683847f1eeb356e554da1e566cd05dd395b` 的 Hosted CI `5/5` 全部通过；
第三十一轮精确 SHA review `4790867860` 确认第三十轮问题未重复，并提出三项：
Combat health/condition 未持久化为新 Character Sheet version；terminal
Combat/Chase 正史提交但状态投影失败后，Session 结束会阻止 exact retry 恢复投影；
Growth 只更新顶层 skill，未同步 `combat_profile.skill_targets`。

当前最小修复将 Combat health delta、来源版本与确定性新 Sheet ID 纳入 canonical
event，并以保持原 Visibility 的锁定 Sheet version 推进 Character；live、replay、
fork 与 P08 rebuild 共用同一正史边界。Combat/Chase 在 Session gate 前识别经过
HMAC/Witness 验证的精确 canonical retry，并复用已持久化 projection targets，只有
首次请求仍要求 ACTIVE。Growth 按显式 `skill_target_sources`（兼容既有标准技能）
同步 combat target，live/replay/fork 一致。新增 forward-only migration 只对正式
Combat event 的精确 HMAC-bound Character/Sheet targets 放行私密投影，并提供受
