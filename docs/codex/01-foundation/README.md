> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/01-foundation/README.md`
...
# 01-foundation — Codex 模块施工目录

范围：Rust workspace / shared kernel / configuration
默认 crate：`trpg-shared-kernel`
默认 module prefix：`shared_kernel`
Per-file prompts：98

## 模块关键要求

workspace 结构、共享 envelope、错误模型、配置加载和依赖方向必须先稳定。
...
- per-file prompts：`codex-prompts/01-foundation/`
