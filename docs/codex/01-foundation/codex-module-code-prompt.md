> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/01-foundation/codex-module-code-prompt.md`
...
# 01-foundation — Codex 模块编码提示词

```text
你是 Codex，负责 `01-foundation` 模块施工。

模块范围：Rust workspace / shared kernel / configuration
默认 crate：trpg-shared-kernel
模块前缀：shared_kernel

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/01-foundation/` 中的 per-file prompt 执行。
3. workspace 结构、共享 envelope、错误模型、配置加载和依赖方向必须先稳定。
