> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/02-domain-core/codex-module-code-prompt.md`
...
# 02-domain-core — Codex 模块编码提示词

```text
你是 Codex，负责 `02-domain-core` 模块施工。

模块范围：Domain core / command / authority / visibility
默认 crate：trpg-domain-core
模块前缀：domain_core

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/02-domain-core/` 中的 per-file prompt 执行。
3. authority、visibility、fact provenance、command idempotency 是全系统写路径的基础。
