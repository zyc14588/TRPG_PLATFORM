> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/12-extension-sdk/codex-module-code-prompt.md`
...
# 12-extension-sdk — Codex 模块编码提示词

```text
你是 Codex，负责 `12-extension-sdk` 模块施工。

模块范围：Plugin SDK / ruleset pack / agent pack / tool provider
默认 crate：trpg-extension-sdk
模块前缀：extension_sdk

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/12-extension-sdk/` 中的 per-file prompt 执行。
3. 插件、规则包、Agent Pack、Tool Provider 必须通过 capability 和 policy gate。
