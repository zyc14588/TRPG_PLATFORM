> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/99-appendix/codex-module-code-prompt.md`
...
# 99-appendix — Codex 模块编码提示词

```text
你是 Codex，负责 `99-appendix` 模块施工。

模块范围：Appendix / templates / glossary / research notes
默认 crate：trpg-docs-governance
模块前缀：appendix

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/99-appendix/` 中的 per-file prompt 执行。
3. 模板、术语和调研资料不得覆盖主线约束，只能作为辅助。
