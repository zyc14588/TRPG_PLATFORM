> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/05-ruleset-coc7/codex-module-code-prompt.md`
...
# 05-ruleset-coc7 — Codex 模块编码提示词

```text
你是 Codex，负责 `05-ruleset-coc7` 模块施工。

模块范围：CoC7 ruleset / dice / combat / sanity / chase
默认 crate：trpg-ruleset-coc7
模块前缀：ruleset_coc7

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/05-ruleset-coc7/` 中的 per-file prompt 执行。
3. 规则实现必须可回放、可测试、可解释，并隔离 keeper-only 事实。
