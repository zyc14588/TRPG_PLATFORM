> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/11-ops-migration/codex-module-code-prompt.md`
...
# 11-ops-migration — Codex 模块编码提示词

```text
你是 Codex，负责 `11-ops-migration` 模块施工。

模块范围：Backup / restore / migration / incident response / release
默认 crate：trpg-ops
模块前缀：ops_migration

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/11-ops-migration/` 中的 per-file prompt 执行。
3. 备份、恢复、迁移、rollback 必须保护 Event Store 正史完整性。
