> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/00-index/codex-prompt-boundary.md`
> 筛选状态：`active-index`
> 清理日期：2026-07-01
...
# Codex Prompt Boundary — 文档权威与提示词边界

## 权威顺序

1. 项目不可变施工原则。
2. `AGENTS.md` 与 `docs/codex/00-index/codex-persistent-context.md`。
3. 模块级 `codex-module-code-prompt.md` / `codex-module-test-prompt.md`。
4. per-file prompt。
5. 旧 V5 文档摘要和摘录。

若 per-file prompt 与全局原则冲突，以全局原则为准；若实现便利与安全约束冲突，以安全约束为准。

## 过时文档处理

本包不复制 V5 原始正文。旧文档只通过 SHA256、摘要、标题、锚点和 prompt 映射保留追踪。Codex 不应从旧 generated 文档恢复模板化代码，而应以 V6 prompt 中的领域专名要求实施。

## 持久化方式

仓库根目录保留 `AGENTS.md`；模块特化要求保留在 `docs/codex/<category>/AGENTS.md` 与模块级提示词中。Codex 分批施工时必须重新读取相关 prompt。
