> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/00-index/codex-module-code-prompt.md`
> 筛选状态：`active-index`
> 清理日期：2026-07-01
...
# 00-index — Codex 模块编码提示词

```text
你是 Codex，负责 `00-index` 模块施工。

模块范围：项目索引与施工总控
默认 crate：trpg-docs-governance
模块前缀：docs_governance

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/00-index/` 中的 per-file prompt 执行。
3. 索引、边界与映射文档必须保持与实现、测试、prompt ID 同步。
