> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/00-index/codex-persistent-context.md`
> 筛选状态：`active-index`
> 清理日期：2026-07-01
> 使用规则：当前可引用：可由 Codex 读取并参与施工，但必须服从顶层设计与 v2.21 阶段门禁。
> 过时信息处理：正文中出现的 `V4`、`V5`、早期 audit/fix/report 标题、源文档 hash、旧中间路径与历史版本流水仅表示 provenance，不得作为当前产品范围、命名规则或验收标准。若与顶层设计、`AGENTS.md`、`docs/construction/CODEX_STANDALONE_BOOTSTRAP_PROMPT.md`、`docs/construction/SOURCE_BUNDLE_INTEGRATION_GUIDE.md` 或 `docs/acceptance/V1_ACCEPTANCE_EVIDENCE_MATRIX.md` 冲突，以后者为准。
...
# Codex Persistent Context — 项目持久化施工上下文

本文件适合作为 Codex `AGENTS.md` 的扩展阅读材料，也可复制到 Codex 任务首轮提示词中。

## 项目目标

实现一个 Rust-first 的 COC AI TRPG 后端与协作系统。系统以 Event Store 为正史，以 Command / Workflow / Decision / Projection 为写入链路，AI 只能作为受约束代理提出建议，不可绕过正式决策链路。
...
| 层 | 默认选择 | Codex 施工要求 |
|---|---|---|
| Rust workspace | Cargo workspace | crate 边界清晰，禁止循环依赖。 |
| Persistence | PostgreSQL + SQLx | 所有正式写入走 transaction、event append、idempotency guard。 |
| Event bus | NATS JetStream adapter | 只发布 Event Store 派生事件，不替代正史。 |
| API | Axum + utoipa | handler 必须传递 actor、visibility、provenance、correlation_id。 |
| AuthZ / policy | OpenFGA + OPA/Rego | 关系授权和上下文策略分层，拒绝绕过。 |
| RAG | pgvector + RagSnapshot | 向量索引是可重建读模型，必须固定 snapshot / embedding model / chunk hash。 |
| Observability | tracing + OpenTelemetry | logs/metrics/traces 不得泄露不可见内容。 |
| Testing | unit + integration + contract + golden replay | 任何写路径必须有正反测试。 |

## Codex 执行姿态

- 先小步修改，再运行局部测试，最后运行 workspace 级验证。
- 遇到设计冲突时，以 `AGENTS.md` 和本文件的不可变约束为最高优先级。
- 不要为了通过测试删除测试、弱化错误、关闭 policy gate 或跳过 migration。
- 未能执行的命令必须记录原因、环境限制和可复现替代命令。
