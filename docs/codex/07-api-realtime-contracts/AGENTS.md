> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/07-api-realtime-contracts/AGENTS.md`
...
# AGENTS.md — 07-api-realtime-contracts Codex 模块规则

本文件是 `07-api-realtime-contracts` 模块的局部 Codex 指令。执行该模块相关代码前，先读取仓库根 `AGENTS.md`，再读取本文件。

## 模块范围

Axum API / OpenAPI / WebSocket / realtime contracts
...
所有 API / WebSocket / NATS 契约必须传播 actor、visibility、provenance 和 correlation。
