> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/07-api-realtime-contracts/codex-module-code-prompt.md`
...
# 07-api-realtime-contracts — Codex 模块编码提示词

```text
你是 Codex，负责 `07-api-realtime-contracts` 模块施工。

模块范围：Axum API / OpenAPI / WebSocket / realtime contracts
默认 crate：trpg-api
模块前缀：api_realtime

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/07-api-realtime-contracts/` 中的 per-file prompt 执行。
3. 所有 API / WebSocket / NATS 契约必须传播 actor、visibility、provenance 和 correlation。
