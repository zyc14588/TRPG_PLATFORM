> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/03-runtime-orchestration/codex-module-code-prompt.md`
...
# 03-runtime-orchestration — Codex 模块编码提示词

```text
你是 Codex，负责 `03-runtime-orchestration` 模块施工。

模块范围：Runtime orchestration / workflow / saga / scheduler
默认 crate：trpg-runtime
模块前缀：runtime_orchestration

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/03-runtime-orchestration/` 中的 per-file prompt 执行。
3. workflow、saga、pending decision 和 session runtime 必须保证正式状态只从 decision 产生事件。
