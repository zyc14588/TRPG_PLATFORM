> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/10-testing-quality/codex-module-code-prompt.md`
...
# 10-testing-quality — Codex 模块编码提示词

```text
你是 Codex，负责 `10-testing-quality` 模块施工。

模块范围：Golden tests / contract tests / replay / model certification
默认 crate：trpg-testing
模块前缀：testing_quality

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/10-testing-quality/` 中的 per-file prompt 执行。
3. Golden/replay/contract/visibility leakage tests 是合并门禁。
