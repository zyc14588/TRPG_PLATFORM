> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/08-platform-infrastructure/README.md`
...
# 08-platform-infrastructure — Codex 模块施工目录

范围：Deployment / object storage / observability / workers
默认 crate：`trpg-platform`
默认 module prefix：`platform_infrastructure`
Per-file prompts：77

## 模块关键要求

部署、worker、对象存储与观测不得泄露不可见事实。
...
- per-file prompts：`codex-prompts/08-platform-infrastructure/`
