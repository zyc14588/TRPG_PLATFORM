> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/08-platform-infrastructure/codex-module-code-prompt.md`
...
# 08-platform-infrastructure — Codex 模块编码提示词

```text
你是 Codex，负责 `08-platform-infrastructure` 模块施工。

模块范围：Deployment / object storage / observability / workers
默认 crate：trpg-platform
模块前缀：platform_infrastructure

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/08-platform-infrastructure/` 中的 per-file prompt 执行。
3. 部署、worker、对象存储与观测不得泄露不可见事实。
