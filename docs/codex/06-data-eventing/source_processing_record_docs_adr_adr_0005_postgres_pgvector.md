...
- RAG snapshot metadata must include source_type, visibility, copyright_status, version, owner, allowed_use, and fact provenance.
...
- Concurrent rebuilds of one `(campaign_id, snapshot_id)` must serialize at the database boundary so one snapshot cannot mix embedding model/dimension generations.
- Production readiness must verify the pgvector extension, snapshot table, lock function and source-binding trigger. This record does not claim an embedding-generation or Agent-context indexing job.
