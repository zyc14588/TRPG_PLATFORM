> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/06-data-eventing/codex-module-code-prompt.md`
...
# 06-data-eventing — Codex 模块编码提示词

```text
你是 Codex，负责 `06-data-eventing` 模块施工。

模块范围：Event Store / SQLx / Outbox / NATS / Projection
默认 crate：trpg-data-eventing
模块前缀：data_eventing

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/06-data-eventing/` 中的 per-file prompt 执行。
3. Event Store 是正史，NATS、projection、cache 都是派生层。
