> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/04-ai-agent-system/codex-module-code-prompt.md`
...
# 04-ai-agent-system — Codex 模块编码提示词

```text
你是 Codex，负责 `04-ai-agent-system` 模块施工。

模块范围：AI agent runtime / model provider / tool protocol / RAG
默认 crate：trpg-agent-runtime
模块前缀：agent_runtime

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/04-ai-agent-system/` 中的 per-file prompt 执行。
3. Agent 只能提出 Proposal / ToolCall / DraftDecision，严禁直接写正式状态。
