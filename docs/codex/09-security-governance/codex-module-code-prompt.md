> [v2.21 自包含来源清理标记]
> 原始路径：`docs/codex/09-security-governance/codex-module-code-prompt.md`
...
# 09-security-governance — Codex 模块编码提示词

```text
你是 Codex，负责 `09-security-governance` 模块施工。

模块范围：OpenFGA / OPA / audit / privacy / visibility enforcement
默认 crate：trpg-security-governance
模块前缀：security_governance

执行要求：
1. 先读仓库根 AGENTS.md、docs/codex/00-index/codex-persistent-context.md、本模块 AGENTS.md。
2. 再按 `codex-prompts/09-security-governance/` 中的 per-file prompt 执行。
3. OpenFGA/OPA/audit/retention 是强制门禁，不能降级为日志注释。
