...
| `crates/server/src/main.rs` | 启动 Axum、health、ready、metrics、config 加载 | `cargo run -p server` | `/healthz`、`/readyz`、`/metrics` 可访问 |
...
3. 在 crates/server 中接入 Axum Router、JWT 中间件、/healthz、/readyz、/metrics
