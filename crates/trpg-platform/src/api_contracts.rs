...
pub const API_CONTRACTS_METRIC_MODULE: &str = "api_contracts";
pub const API_CONTRACTS_REQUIRED_METRICS: &[&str] = &[
    "trpg_command_total",
...
impl From<ApiContractsError> for TrpgError {
    fn from(error: ApiContractsError) -> Self {
        match error {
            ...
            ApiContractsError::SchemaVersionRequired => {
                TrpgError::InvalidConfiguration("schema_version_required")
            }
        }
    }
}
...
impl ApiContractsService {
    pub fn register_api_command_contract(
        ...
        if command.payload.schema_version == 0 {
            return Err(ApiContractsError::SchemaVersionRequired.into());
        }
