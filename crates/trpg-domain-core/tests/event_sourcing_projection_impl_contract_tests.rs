...
use trpg_domain_core::event_sourcing_projection_impl::{
    append_rebuildable_canon_event, rebuild_projection_from_events,
    replay_visible_projection_events,
};
use trpg_domain_core::event_sourcing_snapshot_projection::DomainEventPayload;

#[test]
fn event_sourcing_projection_impl_rejects_authority_violation_without_event() {
    ...
    let error =
        append_rebuildable_canon_event(&mut store, &command, "event_001", "DecisionRecorded")
            .unwrap_err();
...
#[test]
fn event_sourcing_projection_impl_preserves_visibility_and_provenance_on_replay() {
    ...
    .unwrap();
    let mut command =
        trpg_test_support::governed_command("projection", ActorRole::Workflow, AuthorityMode::AiKp);
