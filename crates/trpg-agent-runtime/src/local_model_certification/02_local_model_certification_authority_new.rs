// Issuance and gate logic stays with the authority entry point so the
// certification repair remains within its bounded production-file budget.
...
impl LocalModelCertificationAuthority {
    ...
    fn append_registry_entry(
        ...
        self.with_registry_lock(|observed_head| {
            let records = self.read_verified_registry(observed_head)?;
            self.append_registry_entry_locked(certificate, state, &records, observed_head)
        })
    }

    fn append_registry_entry_locked(
        &self,
        certificate: &LocalModelCertificate,
        state: RegistryState,
