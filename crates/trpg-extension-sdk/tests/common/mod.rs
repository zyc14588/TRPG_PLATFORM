    ...
    ) -> KernelResult<ExtensionEventEnvelope>,
{
    ...
    assert!(contract.forbidden_capabilities.iter().any(|capability| {
        capability.denial_code().as_str() == "EXTENSION_DIRECT_LLM_FORBIDDEN"
    }));
    ...
    match &event.payload {
        ExtensionEvent::ContractRecorded(record) => {
            ...
            assert!(record.evidence_path.starts_with("extensions/"));
            assert!(record
                .capabilities
                .iter()
...
pub fn authority_contract() -> AuthorityContract {
    trpg_test_support::authority_contract("campaign_extension_001", AuthorityMode::AiKp, 1).unwrap()
}
